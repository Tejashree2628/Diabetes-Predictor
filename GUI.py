...
class Diabetes(QWidget):
    ...
    def clfn(self):
        ...
        self.details.setText("We Will Give Best Model.\nHold On..........!!!!\nnWe have used PIMA Indians diabetes dataset from UCI archive.")
        #print(self.frameGeometry().width())
        #print(self.frameGeometry().height())

    def test_input(self) -> None:
        """ test for diabetes"""
        #my_dict = {"Glucose":float(self.l1.text()), "BloodPressure":float(self.l2.text()),"SkinThickness":float(self.l3.text()), "Insulin":float(self.l4.text()), "BMI": float(self.l5.text()), "Age": float(self.l6.text())}
        my_dict = [[float(self.l1.text()),float(self.l2.text()),float(self.l3.text()),float(self.l4.text()),float(self.l5.text()),float(self.l6.text())]]
        output = Diabities_Predector.check_input(my_dict)
        #print(self.output)
        #self.setFixedSize(850, 342)
        self.report_subhead.setText("Report card")
        self.details.setText("Patient's name: {}\nPlasma glucose concentration: {} \
\nDiastolic blood pressure: {}\nTriceps skin fold thickness: {}\nSerum insulin: {}\nBody mass index: {}\nAge : {}".format(self.l0.text(), self.l1.text(), self.l2.text(), self.l3.text(),self.l4.text(),self.l5.text(),self.l6.text()))
        if output==0:
